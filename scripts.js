...
function updateRiskScore(input) {
  ...
  if (row.rowIndex === 1) {
    ...
    row.querySelector(".progress-bar").style.width =
      (riskScore / 3) * 100 + "%";
  }
...
setInterval(() => {
  document.getElementById("total").innerText = totalRiskScore;

  document.querySelector("#totalProgressBar").style.width =
    (totalRiskScore / 18) * 100 + "%";
}, 500);
